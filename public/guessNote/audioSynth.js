...
function AudioSynthView() {
    ...
    function game() {
        ...
        var index = notes.indexOf(note);    //to exclude from options
        if (index !== -1) notes.splice(index, 1);

        var options= getRandomSubarray(notes,3);

        notes.push(note);   //restore 
        options.push(note);
        ...
        var opHtml = '';
        options.forEach(op => {
                opHtml+="<button id='"+op +"'>"+op+"</button>";                
        });
        document.getElementById('options').innerHTML = opHtml;
        ...
        options.forEach(op => {
            document.getElementById(op).addEventListener('click',function(){
                if(op==note){
                    //console.log("Correct");
                    //game();
                    document.getElementById('options').innerHTML = '<h1>Correct</h1>';
                    ...
                    if(life>1){
                        document.getElementById('options').innerHTML = '<h1>In Correct</h1>';   
